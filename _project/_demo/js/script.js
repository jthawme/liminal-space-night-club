...
class FX
{
	...
	maximizeCanvas()
	{
		this.canvas.width = this.doc.clientWidth * 1.5;
		this.canvas.height = this.doc.clientHeight * 1.5;
		this.clearCanvas();
	}
	...
	drawScanLines()
	{
		...
		let y = 0;
		// let lineHeight = 10;
		let lineHeight = Math.round(this.doc.clientWidth / 54);

		while(y < this.doc.clientHeight * 1.5)
		{
			let h = ((Math.sin(p) + 1) * 0.5) * lineHeight;
			this.fillRect(0, y, (this.doc.clientWidth * 1.5), h * this.scale);
			y += lineHeight;
			p += this.scanRate;
		}
	}
	...
	drawVertLines()
	{
		...
		let x = 0;
		// let lineWidth = 10;
		let lineWidth = Math.round(this.doc.clientWidth / 54);

		while(x < this.doc.clientWidth * 1.5)
		{
			let w = ((Math.sin(p) + 1) * 0.5) * lineWidth;
			this.fillRect(x, 0, w * this.scale, (this.doc.clientHeight * 1.5));
			x += lineWidth;
			p += this.scanRate;
		}
	}
	...
	drawHalftone()
	{
		...
		let dotSize = Math.round(this.doc.clientWidth / 48);
		...
		while(y < (this.doc.clientHeight * 1.5)+dotSize)
		{
			x = -dotSize;
			while(x < (this.doc.clientWidth * 1.5)+dotSize)
			{
				let rad = (2 + Math.sin(this.phaseA + (x * 0.004)) + Math.sin(this.phaseB + (y * 0.004))) * 0.25;
				this.ctx.moveTo(x, y);
...
class Quiz
{
	...
	showIdleWarning()
	{
		...
		clearTimeout(this.timer);
		this.timer = setTimeout(this.timeout.bind(this), 10000);
	}
	...
	receiveAnswer(a)
	{
		...
		this.metrics['Question '+(this.currentQuestion+1)] = ''+(parseInt(a.dataset.answer)+1);
	...
	showResults()
	{
		...
		// remove temporary score counter from metrics
		delete this.metrics.scores;
